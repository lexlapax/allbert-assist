defmodule AllbertSignal.SettingsFragment do
  @moduledoc """
  Pack `FragmentOwner` for the Allbert Signal Channel.

  Derived from `AllbertSignal.Settings.Fragment`, which already held the schema and
  ...
  def entries do
    (AllbertSignal.Settings.Fragment.settings_schema() ++ Notify.settings_schema("signal"))
  ...
  def fragment do
    ...
    Fragment.new!(%{
      id: "plugin:allbert.signal",
      owner: "allbert.signal",
      source: :plugin,
      group: :plugins,
      schema: schema,
      defaults: defaults(schema),
      safe_write_keys: Enum.map(safe_write_rows(), &elem(&1, 1)),
      metadata: %{display_name: "Allbert Signal Channel", trust_status: :trusted, source: :shipped}
    })
  end
