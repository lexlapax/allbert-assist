defmodule AllbertAssist.Mcp.ClientTest do
  ...
  setup do
    ...
    root =
      Path.join(
        System.tmp_dir!(),
        "allbert-mcp-client-#{System.pid()}-#{System.unique_integer([:positive])}"
      )
  ...
  defp configure_stdio_server(script) do
    ...
    assert {:ok, _setting} =
             Settings.put(
               "mcp.servers.stdio_demo.enabled",
               false,
               AllbertAssist.TestSupport.ReadyEffectContext.attach(%{audit?: false})
             )
    ...
    assert {:ok, _setting} =
             Settings.put(
               "mcp.servers.stdio_demo.transport",
               "stdio",
               AllbertAssist.TestSupport.ReadyEffectContext.attach(%{audit?: false})
             )
    ...
    assert {:ok, _setting} =
             Settings.put(
               "mcp.servers.stdio_demo.command",
               "elixir",
               AllbertAssist.TestSupport.ReadyEffectContext.attach(%{audit?: false})
             )
