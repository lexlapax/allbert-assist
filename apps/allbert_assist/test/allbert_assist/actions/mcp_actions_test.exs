defmodule AllbertAssist.Actions.McpActionsTest do
  ...
  setup do
    ...
    root =
      Path.join(
        System.tmp_dir!(),
        "allbert-mcp-actions-#{System.pid()}-#{System.unique_integer([:positive])}"
      )
  ...
  defp configure_stdio_tool_server(script, log_path) do
    ...
    assert {:ok, _setting} =
             Settings.put(
               "mcp.servers.stdio_tool.enabled",
               false,
               AllbertAssist.TestSupport.ReadyEffectContext.attach(%{audit?: false})
             )
    ...
    assert {:ok, _setting} =
             Settings.put(
               "mcp.servers.stdio_tool.transport",
               "stdio",
               AllbertAssist.TestSupport.ReadyEffectContext.attach(%{audit?: false})
             )
    ...
    assert {:ok, _setting} =
             Settings.put(
               "mcp.servers.stdio_tool.command",
               "elixir",
               AllbertAssist.TestSupport.ReadyEffectContext.attach(%{audit?: false})
             )
