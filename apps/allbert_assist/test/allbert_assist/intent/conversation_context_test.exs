defmodule AllbertAssist.Intent.ConversationContextTest do
  ...
  defp msg(role, content), do: %{role: role, content: content, inserted_at: "2026-06-16T00:00:00Z", trace_id: nil}
  ...
  test "summarizes only the last context_window turns" do
    messages = [msg("user", "one"), msg("assistant", "two"), msg("user", "three"), msg("assistant", "four")]
  ...
  test "carries antecedent signals when supplied" do
    ctx =
      ...
      |> CC.from_thread_context(multiturn_enabled: true, context_window: 6, prior_app: "stocksage", prior_action: "quote")
