defmodule Mix.Tasks.Allbert.Test.Raw do
  ...
  @impl true
  def run(args) do
    load_ambient_applications()
    Test.run(args)
  end

  # v1.4 M13.2. The closed projection reconciles metadata for every application
  # in the release, and `Application.app_dir/2` raises for one that is built but
  # not loaded. The applications ABOVE Web -- artifacts and stocksage since M13
  # -- are in no owner's dependency closure, so an owner-scoped VM never loads
  # them on its own, while `Test.run/1` starts a closure that reaches the
  ...
  defp load_ambient_applications do
    if Code.ensure_loaded?(AllbertAssist.TestSupport.PackBootstrap) do
      AllbertAssist.TestSupport.PackBootstrap.ensure_ambient_loaded!()
    end
  end
end
