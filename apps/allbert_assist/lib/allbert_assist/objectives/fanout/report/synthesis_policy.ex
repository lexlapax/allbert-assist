defmodule AllbertAssist.Objectives.Fanout.Report.SynthesisPolicy do
  ...
  @rules [
    ...
    %{
      ...
      instruction:
        "Choose relationship sections supported by the supplied observations and explain the substantive cross-child relationship in advisory_synthesis. A section covering exactly one queue position must always use independent, and every other relationship requires at least two positions; this overrides every other consideration below. When the original parent request already frames the relationship — asking to contrast, to explain how things complement or support each other, or to order stages — use the relationship that framing names, because the operator has stated what they are asking for. Otherwise choose each section's relationship by how its observations stand to each other: complementary when they cover different aspects that together give a fuller picture and neither depends on the other; contrasting when they differ, disagree, or set out trade-offs; sequential when they describe stages that follow one another in order; supporting when one observation provides evidence or grounding for another; independent when they have no substantive relationship, which is also the correct choice for a section holding a single observation that stands alone. Group observations that stand in the same relationship into one section, and give a subset its own section when the request or the observations mark it as standing apart from the rest."
    },
    %{
