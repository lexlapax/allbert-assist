defmodule AllbertAssist.Actions.Memory.ConfirmManualMemoryRevision do
  ...
  use AllbertAssist.Action,
    ...
    resumable?: true,
    name: "confirm_manual_memory_revision",
    description: "Preview and confirm one exact raw manual Memory revision.",
    category: "memory",
    tags: ["memory", "repair", "confirmation"],
    schema: [
  ...
  defp preview(claim_id, user_id, context, decision) do
    ...
    with {:ok, prepared} <- ClaimConfirmation.prepare_manual(claim_id, actor),
         {:ok, confirmation} <-
  ...
  defp create_confirmation(claim_id, user_id, binding, context, decision) do
    Confirmations.create(
      %{
        origin: origin(context, user_id),
        target_action: %{name: "confirm_manual_memory_revision", module: inspect(__MODULE__)},
  ...
  defp confirm(binding, claim_id, decision) when is_map(binding) do
    case ClaimConfirmation.confirm_manual(binding) do
      {:ok, result} ->
        {:ok,
         %{
           message: "Confirmed exact manual Memory revision for claim #{claim_id}.",
           status: :completed,
           permission_decision: decision,
           result: result,
  ...
  defp error(decision, reason) do
    {:ok,
     %{
       message: "Unable to confirm manual Memory revision: #{inspect(reason)}",
       status: :error,
       error: reason,
       permission_decision: decision,
