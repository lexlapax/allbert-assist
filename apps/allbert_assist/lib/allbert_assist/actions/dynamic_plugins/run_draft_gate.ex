defmodule AllbertAssist.Actions.DynamicPlugins.RunDraftGate do
  @moduledoc """
  Internal action for running v0.37 draft warning-gate evidence through v0.36 sandbox.
  """
  ...
  use AllbertAssist.Action,
    ...
    name: "run_dynamic_draft_gate",
    description: "Run sandbox warning-gate evidence for a dynamic draft.",
    category: "dynamic_plugins",
    tags: ["dynamic-plugins", "sandbox", "gate", "internal"],
    schema: [
  ...
  def run(params, context) do
    ...
    with true <- PermissionGate.allowed?(permission_decision),
         {:ok, result} <- DynamicPlugins.run_draft_gate(params.slug, bridge_opts(params, context)) do
  ...
  defp completed(permission_decision, result) do
    %{
      message: "Dynamic draft gate finished with status #{result.status}.",
  ...
  defp denied(permission_decision) do
    %{
      message: "Dynamic draft gate is denied by Security Central.",
  ...
  defp failed(permission_decision, reason) do
    %{
      message: "Could not run dynamic draft gate: #{inspect(reason)}",
      status: :denied,
      permission_decision: permission_decision,
      error: reason,
