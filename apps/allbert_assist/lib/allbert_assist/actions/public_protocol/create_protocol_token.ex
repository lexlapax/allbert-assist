defmodule AllbertAssist.Actions.PublicProtocol.CreateProtocolToken do
  @moduledoc """
  ...
  the value stays renderable from the in-memory response yet is never written to
  logs. Action metadata carries only redacted/reference fields — never the raw
  token.
  """
  ...
  use AllbertAssist.Action,
    ...
    name: "create_protocol_token",
    description: "Issue a public-protocol bearer token for a client (secret write).",
  ...
  defp completed(result, permission_decision) do
    {:ok,
     %{
       message: "Issued public protocol bearer token for #{result.surface}/#{result.client_id}.",
       status: :completed,
       permission_decision: permission_decision,
       # The `token` key inside `token_result` is redacted by the Redactor in
       # every logged signal/audit; the in-memory response keeps the raw value
       # so the CLI can print it once.
       token_result: result,
  ...
  defp failed(surface, client, permission_decision, reason) do
    {:ok,
     %{
       message:
         "Public protocol token issuance failed for #{surface}/#{client}: #{inspect(reason)}",
       status: :failed,
       permission_decision: permission_decision,
       error: reason,
