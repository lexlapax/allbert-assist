defmodule AllbertAssist.Actions.Mcp.ScanResume do
  @moduledoc "Resume the managed MCP discovery scan job through the action spine."
  ...
  use AllbertAssist.Action,
    ...
    description: "Resume the managed MCP discovery scan job.",
    category: "mcp",
  ...
  def run(params, context) do
    ...
    with {:allowed, true} <- {:allowed, PermissionGate.allowed?(permission_decision)},
         {:ok, job} <-
           Scan.resume(%{user_id: user_id, operator_id: operator_id(params, context, user_id)}) do
  ...
  defp failed(permission_decision, user_id, reason) do
    {:ok,
     %{
       message: "MCP discovery scan resume failed: #{inspect(reason)}",
       status: :failed,
       permission_decision: permission_decision,
       error: reason,
