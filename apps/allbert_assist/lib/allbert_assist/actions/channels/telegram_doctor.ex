defmodule AllbertAssist.Actions.Channels.TelegramDoctor do
  @moduledoc false
  ...
  use AllbertAssist.Action,
    ...
    description: "Check the configured Telegram channel without exposing secrets.",
    category: "channels",
    tags: ["channels", "telegram", "doctor", "read_only"],
    schema: [],
    output_schema: [
  ...
  defp failed(permission_decision, reason) do
    %{
      message: "Telegram doctor failed.",
      status: :error,
      doctor: %{},
      diagnostics: [reason],
      permission_decision: permission_decision,
  ...
  defp message(result) do
    [
      "Telegram doctor: status=#{result.status}",
      "auth_ok=#{result.auth_ok}",
      "endpoint_ok=#{result.endpoint_ok}",
