defmodule AllbertAssist.Actions.Channels.MatrixDoctor do
  @moduledoc false
  ...
  use AllbertAssist.Action,
    ...
    name: "matrix_doctor",
    description: "Check the configured Matrix channel without exposing secrets.",
    category: "channels",
    tags: ["channels", "matrix", "doctor", "read_only"],
    schema: [],
    output_schema: [
  ...
  defp message(result) do
    [
      "Matrix doctor: status=#{result.status}",
      "auth_ok=#{result.auth_ok}",
      "endpoint_ok=#{result.endpoint_ok}",
      "poller=#{result.poller_status}",
      "user=#{Map.get(result, :user_id, "unknown")}",
