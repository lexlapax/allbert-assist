defmodule AllbertAssist.Actions.Channels.SlackDoctor do
  @moduledoc false
  ...
  use AllbertAssist.Action,
    ...
    name: "slack_doctor",
    description: "Check the configured Slack channel without exposing secrets.",
    category: "channels",
    tags: ["channels", "slack", "doctor", "read_only"],
    schema: [],
    output_schema: [
    ...
    %{
      ...
      actions: [
        action(:completed, permission_decision, %{
          ...
          socket_mode_status: result.socket_mode_status,
          diagnostics: result.diagnostics
        })
      ]
    }
  ...
  defp message(result) do
    [
      "Slack doctor: status=#{result.status}",
      "auth_ok=#{result.auth_ok}",
      "endpoint_ok=#{result.endpoint_ok}",
      "socket_mode=#{result.socket_mode_status}",
      "team=#{Map.get(result, :team, "unknown")}",
