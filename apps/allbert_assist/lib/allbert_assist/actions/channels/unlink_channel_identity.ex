defmodule AllbertAssist.Actions.Channels.UnlinkChannelIdentity do
  @moduledoc """
  v0.62 M8.15 — remove an explicit cross-channel identity link on the one action
  spine.

  Config-level identity mapping, so gated by `:settings_write` and audited
  through the Runner; the delete itself is delegated to
  `Conversations.ChannelThread.unlink_identity/1`.
  """
  ...
  use AllbertAssist.Action,
    ...
    name: "unlink_channel_identity",
    description: "Remove an explicit cross-channel identity link (gated + audited).",
    category: "channels",
    tags: ["channels", "identity", "unlink"],
    schema: [
  ...
  def run(params, context) do
    ...
    with true <- PermissionGate.allowed?(permission_decision),
      ...
      {:ok,
       %{
         message: "Unlinked #{link.link_id} #{link.channel} identity.",
         status: :completed,
  ...
  defp denied(attrs, permission_decision, reason) do
    %{
      message: "I could not unlink the channel identity: #{inspect(reason)}",
  ...
  defp attr_metadata(attrs) do
    %{link_id: Map.get(attrs, :link_id), channel: Map.get(attrs, :channel)}
  end
end
