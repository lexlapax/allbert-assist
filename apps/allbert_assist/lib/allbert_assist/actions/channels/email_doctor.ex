defmodule AllbertAssist.Actions.Channels.EmailDoctor do
  @moduledoc false
  ...
  use AllbertAssist.Action,
    ...
    name: "email_doctor",
    description: "Check the configured email channel without exposing secrets.",
    category: "channels",
    tags: ["channels", "email", "doctor", "read_only"],
    schema: [],
    output_schema: [
    ...
    %{
      ...
      actions: [
        action(:completed, permission_decision, %{
          ...
          poller_status: result.poller_status,
          diagnostics: result.diagnostics
        })
      ]
    }
  ...
  defp message(result) do
    [
      "Email doctor: status=#{result.status}",
      "auth_ok=#{result.auth_ok}",
      "endpoint_ok=#{result.endpoint_ok}",
      "poller=#{result.poller_status}",
      "imap=#{result.imap_endpoint_ok}",
      "smtp=#{result.smtp_endpoint_ok}",
