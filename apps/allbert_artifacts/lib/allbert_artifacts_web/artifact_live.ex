defmodule AllbertArtifactsWeb.ArtifactLive do
  ...
  `:live_view` -- see that behaviour's moduledoc for why. `mount/3` and
  `handle_params/3` keep their signatures unchanged; only the entry point moved.
  """
  ...
  # The generic `/apps/:pack/*surface_path` route binds the sha as the sole
  # `surface_path` segment (the old route bound it as `:sha` directly). Both
  # keys are accepted so a caller still holding the pre-M13 params shape --
  # a direct `handle_params/3` call from a test, say -- keeps working.
  @impl true
  def handle_params(%{"surface_path" => [sha | _rest]}, _uri, socket) do
    handle_sha_params(sha, socket)
  end
  ...
  defp handle_sha_params(sha, socket) do
    if valid_sha?(sha) do
