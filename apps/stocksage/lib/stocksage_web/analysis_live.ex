defmodule StockSageWeb.AnalysisLive do
  ...
  v1.4 M13: hosted by `AllbertAssistWeb.PackSurfaceLive` instead of routed
  directly, so this declares `AllbertAssist.Pack.WebSurface` and uses `:html`
  (Phoenix.Component, for `~H` and the shared components) rather than
  `:live_view` -- see that behaviour's moduledoc for why. `mount/3`,
  `handle_params/3`, `handle_event/3` and `handle_info/2` keep their signatures
  unchanged; only the entry point moved. `stream/3,4`, `stream_insert/4` and
  `connected?/1` live in `Phoenix.LiveView` rather than `Phoenix.Component`, so
  they are imported narrowly below.
  """
  ...
  def handle_params(params, _uri, socket) do
    analysis_id = analysis_id_from_params(params)
  ...
  # The generic `/apps/:pack/*surface_path` route binds the analysis id as the
  # second `surface_path` segment, after the literal "analyses" segment (the
  # old route bound it as `:id` directly): `/apps/stocksage/analyses/<id>`
  # arrives as `surface_path: ["analyses", id]`, and the bare
  # `/apps/stocksage/analyses` -- unreachable under the old route, which
  # required the id segment -- arrives as `surface_path: ["analyses"]` and
  # renders the index the same way a missing id always has. The old `"id"` key
  # is still accepted so a caller still holding the pre-M13 params shape -- a
  # direct `handle_params/3` call from a test, say -- keeps working.
  defp analysis_id_from_params(%{"surface_path" => [_analyses, id | _rest]}), do: id
  defp analysis_id_from_params(%{"surface_path" => [_analyses]}), do: nil
  defp analysis_id_from_params(%{"id" => id}), do: id
  defp analysis_id_from_params(_params), do: nil
