defmodule AllbertMatrix.SettingsFragment do
  @moduledoc """
  Pack `FragmentOwner` for the Allbert Matrix Channel.

  Derived from `AllbertMatrix.Settings.Fragment`, which already held the schema and
  ...
  def entries do
    (AllbertMatrix.Settings.Fragment.settings_schema() ++ Notify.settings_schema("matrix"))
  ...
  def fragment do
    ...
    Fragment.new!(%{
      id: "plugin:allbert.matrix",
      owner: "allbert.matrix",
      source: :plugin,
      group: :plugins,
      schema: schema,
      defaults: defaults(schema),
      safe_write_keys: Enum.map(safe_write_rows(), &elem(&1, 1)),
      metadata: %{display_name: "Allbert Matrix Channel", trust_status: :trusted, source: :shipped}
    })
  end
