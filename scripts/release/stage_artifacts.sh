#!/usr/bin/env bash
# Stateless v1.2.3 source-run artifact selection and digest-manifest composer.
...
qualify_fv() {
  ...
  [ -x "$harness" ] || {
    echo "stage-artifacts: bounded v1.2.3 TUI/FV harness is unavailable: $harness" >&2
    exit 1
  }
