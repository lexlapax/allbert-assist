defmodule StockSageWeb.QueueLive do
  ...
  def mount(_params, _session, socket) do
    {:ok, Live.assign_context(socket, :stocksage_queue)}
  end
  ...
  def render(assigns) do
    ...
    >
      ...
      <section :if={@web_enabled?} class="mx-auto flex max-w-6xl flex-col gap-5">
        ...
        <section id="stocksage-queue-empty" class="rounded border border-zinc-800 bg-zinc-900 p-5">
          <h2 class="text-lg font-semibold">No queued analyses loaded</h2>
          <p class="mt-2 max-w-2xl text-sm text-zinc-300">
            Queue entries will render with existing list and table primitives in v0.27.
          </p>
        </section>
      </section>
    </main>
