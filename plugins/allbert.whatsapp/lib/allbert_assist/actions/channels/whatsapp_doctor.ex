defmodule AllbertAssist.Actions.Channels.WhatsAppDoctor do
  @moduledoc false
  ...
  use AllbertAssist.Action,
    ...
    description: "Check the configured WhatsApp Cloud API channel without exposing secrets.",
    category: "channels",
    tags: ["channels", "whatsapp", "doctor", "read_only"],
    schema: [],
    output_schema: [
  ...
  defp message(result) do
    [
      "WhatsApp doctor: status=#{result.status}",
      "auth_ok=#{result.auth_ok}",
      "endpoint_ok=#{result.endpoint_ok}",
      "adapter=#{result.adapter_status}",
      "phone=#{Map.get(result, :phone_number_id, "unknown")}",
