...
- hardened Ollama/Gemma4 turns by disabling hidden thinking in chat requests and surfacing empty length-truncated Ollama responses as explicit provider errors instead of blank assistant turns
- fixed Telegram identity resolution so a documented chat-id binding also matches runtime sender keys that include both chat id and Telegram user id
