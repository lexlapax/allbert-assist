...
Release.** Tag `v1.2.5` points to `f4a04858`; source workflow run
[30479193128](https://github.com/lexlapax/allbert-assist/actions/runs/30479193128)
built and smoked all three native archives and passed the Linux install,
service, vault, and uninstall rehearsal. Digest-manifest composition then
rejected setup-beam's recorded `v1.19.5-otp-28` distribution because the
validator still synthesized `otp-29` from the independently verified OTP
29.0.1 runtime. Downstream license/TUI qualification and promotion were skipped
and no GitHub Release was created. The tag is never moved or overwritten.
...
Status: **failed immutable attempt — no artifacts and no GitHub Release.** Tag
`v1.2.2` points to `15c15484`; source workflow run
[30474512758](https://github.com/lexlapax/allbert-assist/actions/runs/30474512758)
stopped all three native builders before packaging. The absolute Phoenix output
path was correct, but nested umbrella Mix structure rebuilt five tracked logical
`.gz` files before the cleanliness assertion. The run therefore published
nothing, and the tag is never moved or overwritten.
...
- **Trusted install and restore.** The curl installer now downloads
  ...
  signing as optional, and its pre-publish Linux rehearsal now signs the local checksum
  bundle before running the real installer. Upgrade backups can be listed and restored
  through the bounded, confirmation-gated `restore_database_backup` action and `allbert
  admin db list-backups|restore`.
...
Status: **tagged for artifact validation, not published**. `v0.64.0` was tagged on
2026-07-09 at version 0.64.0 after `mix allbert.test release.v064` passed, but the
release-artifacts workflow stopped before GitHub Release publication because the
pre-publish Linux rehearsal did not create a cosign bundle for its local `file://`
installer path. The tag is retained as evidence of the blocked validation run and is
superseded by `v0.64.1`.
...
Status: validation complete and release-ready for `0.53.0` through M11. Telegram
and email delivery/inbound plus manual approval/rejection/poll-resume validation
passed after the v0.54 router prerequisite landed. Matrix delivery, inbound,
mapped approval, and unmapped callback rejection validation passed on 2026-06-18
after Matrix sync/catch-up remediations; the release owner accepted the
encrypted-room exclusion as validated for this pass. Discord and Slack
delivery/inbound regression smokes passed after M11 shared-channel changes.
WhatsApp Cloud API is implemented, but not released for
...
- Roadmap, request-flow, security-hardening, operator, developer, and
  ...
  Telegram, email, Matrix, Discord, and Slack validation complete.
...
- WhatsApp Cloud API and Signal live smokes are future/provider runbooks, not
  ...
  by `whatsapp post-webhook` and deterministic eval rows.
- Discord/Slack remain released from v0.52 and passed v0.53 M11 closeout
  regression for shared channel plumbing; they are not new v0.53 feature scope.
  Slack delivery initially returned `not_in_channel` until the bot was correctly
  invited into the target channel; after setup correction, delivery and inbound
  smokes passed.
- Final closeout gates passed: `mix allbert.test release.v053`,
  `mix allbert.test docs`, and `git diff --check`.
...
- The corrective tag reports `0.51.3` across the umbrella, core app, web app,
  README, and `AllbertAssist.App.CoreApp.version/0`; after merge to main,
  active version metadata remains `0.52.0`.
...
- The corrective tag reports `0.51.2` across the umbrella, core app, web app,
  README, and `AllbertAssist.App.CoreApp.version/0`; after merge to main,
  active version metadata remains `0.52.0`.
