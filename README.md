...
- schema-bound intent routing over `{task, chat, schedule, memory_query, meta}`
- router-guided scheduling and explicit-memory capture that keep durable job confirmation and staged-memory review intact
...
- optional diagnosis remediation that requires `self_diagnosis.allow_remediation`, an explicit `--remediate <code|skill|memory> --reason <text>`, and the existing review surfaces; concrete candidate generation is reconciled in v0.14.1
...
- since v0.14.2, there is no compatibility facade for the old Rust `allbert_kernel` API
...
- self-diagnosis explains by default; remediation is opt-in and always routes through existing review surfaces; concrete candidate generation is available in the current release after v0.14.1 reconciliation
