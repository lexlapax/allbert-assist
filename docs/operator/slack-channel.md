...
Then run the live inbound smoke. It opens the real Slack Socket Mode session,
waits for `hello`, prints an exact marker message, and waits for the mapped
user's @mention to reach `Runtime.submit_user_input/1`. Set a generous timeout
and (optionally) a known marker, since stdout is buffered when captured:

```sh
export ALLBERT_TEST_KEEP_TMP=1
export ALLBERT_MESSAGING_CHANNEL_INBOUND_TIMEOUT_MS=600000
export ALLBERT_SMOKE_MARKER="allbert-v052-slack-check"
export ALLBERT_SLACK_BOT_TOKEN="..."
