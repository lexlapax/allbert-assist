...
- **Growth-loop ingestion and browser capture.** Ambient user-activity ingestion, CLI piped ingest, browser extension capture, browser proxy/PAC mode, staged ingestion review, and ingestion-driven learning refresh are parked in [future-plans.md](future-plans.md). They should return only after v0.15 proves the RAG substrate over existing docs, commands, settings, memory, and session recall.
