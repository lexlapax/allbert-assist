...
| v0.15 | RAG, recall, and help foundation: SQLite lexical/vector retrieval substrate, router RAG hints, operator docs/help/settings retrieval, memory/fact/episode recall, and session indexing boundary | Stub | [v0.15-rag-recall-help.md](v0.15-rag-recall-help.md) |
...
v0.15 stays focused on the RAG foundation itself. That ordering matters: browser/search ingestion would create more staged and promoted knowledge than Allbert can retrieve or explain well if the system still only had memory-local BM25 plus the v0.11 fake semantic seam. The v0.15 RAG substrate uses SQLite lexical retrieval first, optional vector retrieval behind an owned embedding seam, bounded RAG hints for the v0.14.3 router, and explicit recall over durable memory, approved facts, labelled episodes, and session-derived history. Growth-loop ingestion, CLI feed, browser extension capture, browser proxy/PAC mode, staged ingestion review, and ingestion-driven learning refresh are preserved as parked stubs in [future-plans.md](future-plans.md).
