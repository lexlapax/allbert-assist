...
| v0.13 | Local personalization: LoRA/adapter training through the v0.11 `LearningJob` seam, owned `AdapterTrainer` trait with mlx + llama.cpp + fake backends, local-only base-model-pinned activation, `adapter-approval` inbox kind, daily wall-clock compute cap, profile-export exclusion | Draft | [v0.13-personalization.md](v0.13-personalization.md) |
