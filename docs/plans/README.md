# Plans

Plans describe release scope, request flow, validation, and handoff. Use the
[roadmap](roadmap.md) as the canonical planning index; it links the versioned
plan and request-flow documents.

## Current And Next

- [Roadmap](roadmap.md)
- [Allbert Jido vision](allbert-jido-vision.md)
- [Future features](future-features.md)
- [v0.58 plan](v0.58-plan.md)
- [v0.58 request flow](v0.58-request-flow.md)
- [v0.59 plan](v0.59-plan.md)
- [v0.59 request flow](v0.59-request-flow.md)
- [v0.60 plan](v0.60-plan.md)
- [v0.60 request flow](v0.60-request-flow.md)
- [v0.60b plan](v0.60b-plan.md)
- [v0.60b request flow](v0.60b-request-flow.md)
- [v0.61 plan](v0.61-plan.md)
- [v0.61 request flow](v0.61-request-flow.md)
- [v0.61b plan](v0.61b-plan.md)
- [v0.61b request flow](v0.61b-request-flow.md)
- [v0.62 plan](v0.62-plan.md)
- [v0.62 request flow](v0.62-request-flow.md)
- [v0.62b plan](v0.62b-plan.md)
- [v0.62b request flow](v0.62b-request-flow.md)
- [v0.63 plan](v0.63-plan.md)
- [v0.63 request flow](v0.63-request-flow.md)
- [v0.64 plan](v0.64-plan.md)
- [v0.64 request flow](v0.64-request-flow.md)
- [v0.65 plan](v0.65-plan.md)
- [v0.65 request flow](v0.65-request-flow.md)
- [v0.66 plan](v0.66-plan.md)
- [v0.66 request flow](v0.66-request-flow.md)
- [v1.0 plan](v1.0-plan.md)
- [v1.0 request flow](v1.0-request-flow.md)
- [v1.0 handoff](v1.0-handoff.md)

## Conventions

- `vX.Y-plan.md` defines release scope, milestones, dependencies, and gates.
- `vX.Y-request-flow.md` defines operator flows, validation, and evidence.
- Historical version plans remain in this directory for auditability.
- Superseded broad planning notes live in [archives](../archives/README.md).

## Release Status

Use [CHANGELOG.md](../../CHANGELOG.md) for shipped release details. Use active
plans and request flows for current implementation scope.
