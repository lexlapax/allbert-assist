...
v1.2.4 thin-client presentation amendment accepted 2026-07-28).
Date: 2026-06-21
...
ADR 0091 is the additive v1.2.4 successor for process and terminal ownership.
...
daemon snapshot, the thin client uses raw/alternate-screen mode for the attached
...
The v1.2.4 successor also relocates the v1.2 M9 compare-and-write described
...
alternate-screen constraint for the v1.2.4 thin terminal client after its
...
through `Actions.Runner.run/3` + Security Central) so v0.57 can add the named
**local-coding operator** trust tier (ADR 0056 lineage, running at sandbox
Level 1 — not "level 0"; see ADR 0068 and the ADR 0009 v0.57 clarification) on the
SAME adapter/channel without weakening the boundary. Pi-mode runs IN the same
persistent TUI channel session, hardened by the v0.55.1 operator/validation
console, not a new channel.
...
- A fresh built-in-profile local launch durably enables the TUI and maps it to
  ...
  state persists across TUI restarts. In v1.2.4 this setup is admitted and
