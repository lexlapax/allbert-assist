...
2. **OS spawns capture their kill handle at spawn time.** The execution
   runners (`Execution.LocalRunner`, `SkillScriptRunner`, sandbox
   `command`/`container_runner` backends) move from bare `System.cmd/3` to a
   port-based spawn that records the OS pid and starts the child as a
   process-group leader (`setsid` wrapper where the platform provides it;
   recorded-pid fallback where not). The kill handle lives with the
   execution record for the duration of the run. Container-backed sandbox
   execution kills via the container runtime's own stop/kill (the group rule
   applies inside the container namespace).
...
- The delegate-agent substrate is unchanged in v1.1: a `:delegate_agent`
  ...
  scope.
- Platform nuance: process-group semantics differ on macOS/Linux (both
  Tier-1 hosts) — the M4 suite runs the kill matrix on both; Windows/WSL2
  stays out of scope with the standing WSL2 deferral.

## Validation

- v1.1 M4, red-first: (a) the ORPHAN PROOF on pre-fix code — a
  spawn-a-sleeping-child fixture whose OS pid survives today's timeout path;
  (b) post-fix: the same fixture's group is dead after tier-3 cancel AND
  after timeout, on macOS and Linux; (c) tier-1 checkpoint cancel completes
  the current operation and stops cleanly; (d) tier-2 drain preserves the
  last durable step; (e) scope: a sibling run's child process survives an
  adjacent cancel untouched.
