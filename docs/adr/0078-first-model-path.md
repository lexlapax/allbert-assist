...
Amended in v1.3 M9.b.3 (2026-07-31) to separate first-model substrate
readiness from DirectAnswer task qualification.
Date: 2026-06-30
...
## Amendment (v1.3 M9.b.3, 2026-07-31) — first model and DirectAnswer are separate

The curated first model and global `local` profile remain
`local_ollama` / `llama3.2:3b`. They continue to prove the broad
consumer-default substrate: the runtime is reachable, a modest local model is
pulled, and the operator can reach general chat and repair surfaces. They do
not, by themselves, prove that the flagship DirectAnswer path meets its
semantic-usefulness contract.

DirectAnswer therefore ships a separate task profile,
`direct_answer_local` / `qwen2.5:7b`, with deterministic temperature `0`,
`max_tokens: 1024`, and `timeout_ms: 60_000`. The DirectAnswer task preference
defaults to `["direct_answer_local"]`; a non-empty list is closed and
authoritative in operator-authored order, while an empty list retains the legacy global-
primary compatibility fallback. Selecting or repairing this task profile does
not silently change the curated/global profile.

First-run detection still reports the six existing first-model substrate
states. Enablement independently verifies the head of the DirectAnswer task
chain. A healthy `llama3.2:3b` substrate with missing `qwen2.5:7b` therefore
projects `enabled_unavailable` and an explicit select/pull-Qwen repair; it does
not claim DirectAnswer ready, fall back silently to `local`, reinstall Ollama,
or pull a model without confirmation. An explicitly authored hosted
DirectAnswer head remains eligible only with the existing egress disclosure.
An unrelated hosted credential is not an implicit task candidate.

Qwen model weights remain an operator-managed external Ollama download. They
are not bundled into the Allbert binary, generated by Allbert, or included in
the binary component-license inventory. The catalog records the model's
license and upstream metadata for operator inspection; the installed model
remains governed by its upstream terms. This preserves the existing no-bundled-
runtime/no-bundled-model decision and adds no license-management subsystem.
