...
existing channel boundary and the v0.51 approval-primitive amendment:
...
## v0.51 Amendment: Channel Approval Primitives

Status: Proposed for v0.51 Channel Pack 1 - Discord And Slack
(`docs/plans/v0.51-plan.md`). Becomes binding for v0.51 and all later
channel adapters including v0.52 mobile channels.

M5 closeout flips this line to `Accepted for v0.51 Channel Pack 1 -
...
v0.51 (Discord + Slack), the field needs Discord buttons / Slack Block Kit;
v0.52 mobile channels need WhatsApp typed commands, Signal typed commands,
...
Adapter declarations as of v0.52:
...
`<Plugin>.channels/0`, in a new `primitives:` field added at v0.51:

```elixir
%{
  channel_id: "telegram",
  # ...existing v0.17 descriptor fields...
  primitives: [:button, :typed_command, :list]   # v0.51 amendment
}
```
...
hand-rolling their choice (v0.51 M0).
...
- No bypass of `Actions.Runner.run/3`, Security Central, or the durable
  confirmation store.
- No primitive that returns rich operator input beyond the three v0.51
...
- v0.51 lands Discord and Slack with `:button` rendering through the same
  Approval Handoff path as Telegram.
- v0.52 lands WhatsApp/Signal/Matrix with explicit primitive declarations
  rather than ad-hoc rendering.
- The v0.53 cross-surface eval sweep adds an `approval-primitive-honor`
  check per adapter.
