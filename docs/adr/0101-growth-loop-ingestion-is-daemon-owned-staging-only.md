...
The v0.15 stub now names ambient ingestion as the next major release after the v0.14.1 repair pass, v0.14.2 structural split, and v0.14.3 router reliability work. v0.15 also adds a RAG foundation first ([ADR 0106](0106-rag-index-is-a-derived-sqlite-lexical-vector-store.md)) so ingestion has a bounded retrieval substrate before new staged records start arriving. This ADR records the provisional ingestion posture until the full v0.15 plan is drafted.
...
- The v0.15 RAG foundation gives promoted ingest records a retrieval path without letting unreviewed staged ingest leak into ordinary turns.
...
- **Build ingestion before RAG.** Rejected for v0.15 ordering because ingestion would create a larger review and memory corpus before Allbert has a unified retrieval substrate for help, memory, and promoted ingest records.
