...
`trace.jsonl` is append-only and line-oriented. Each completed span is written as one JSON object with an Allbert-owned schema version, span id, parent id, session id, trace id, span name, kind, timestamps, duration, status, attributes, and events. The schema tolerates additive attributes and events; schema-breaking changes require a version bump and reader compatibility tests.
...
Rotation is per session. When the active file exceeds `trace.session_disk_cap_mb`, it rotates to `trace.<n>.jsonl.gz` and a fresh `trace.jsonl` starts. Per-session cap enforcement removes oldest rotated trace archives first. Total trace cap enforcement removes trace artifacts from oldest sessions first. Trace GC may remove only trace artifacts: it must not remove `turns.md`, `meta.json`, approvals, patch artifacts, attachments, or other session files.
