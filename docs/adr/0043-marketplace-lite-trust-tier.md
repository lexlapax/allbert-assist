...
- Stable identity for trace/audit (the URI appears in event
  metadata).
- Reference target for v0.46 self-improvement trace mining.
...
convention at v0.45 but accepts the migration runner at v0.51. v0.45
...
- **ADR 0046** (settings schema migration policy): drafted at v0.45;
  accepted at v0.51. v0.45 adopts the `schema_version` field
  convention only.
...
convention before v0.51 ships the runtime migration tool.

v0.46 self-improvement gets a stable trace pattern source
