...
- `:marketplace_schema_version_mismatch` — settings
  ...
  version (preview of ADR 0046's v0.53 runtime migration
  semantics).
...
convention at v0.45 but accepts the migration runner at v0.53. v0.45
...
- **ADR 0046** (settings schema migration policy): drafted at v0.45;
  accepted at v0.53. v0.45 adopts the `schema_version` field
  convention only.
...
convention before v0.53 ships the runtime migration tool.
