...
new settings fragments). Accepted at v0.57 Hardening + Export/Import + Final
RC where the migration tool ships.

### v0.45 vs v0.57 split
...
v0.57 ships the **runtime migration tool** + bump semantics +
...
a migration policy because export from v0.40 + import on v0.57 must work.
...
v0.57 export/import preserves each fragment's `schema_version` so a profile
exported from v0.43 and imported on v0.57 is correctly identified as needing
...
- v0.57 export/import has a binding contract for what schema_version metadata
  to include.
...
- **v0.57 Hardening**: ADR 0046 accepted; `mix allbert.settings.migrate`
