...
new settings fragments). Accepted at v0.52 Hardening + Export/Import + Final
RC where the migration tool ships.

### v0.45 vs v0.52 split
...
v0.52 ships the **runtime migration tool** + bump semantics +
...
a migration policy because export from v0.40 + import on v0.52 must work.
...
v0.52 export/import preserves each fragment's `schema_version` so a profile
exported from v0.43 and imported on v0.52 is correctly identified as needing
...
- v0.52 export/import has a binding contract for what schema_version metadata
  to include.
...
- **v0.47-v0.51 milestones**: each new fragment SHOULD declare
...
- **v0.52 Hardening**: ADR 0046 accepted; `mix allbert.settings.migrate`
