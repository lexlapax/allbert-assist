...
`registered_module?/1`, `capabilities_for_app/1`, `app_id_for_action/1`, and
`diagnostics/0`. Display/resolution order is static reviewed actions, reviewed
source-tree plugin/app actions, then integrated dynamic actions. The overlay
denies collisions rather than shadowing static, plugin, app, or other dynamic
actions; a denied contribution appears in diagnostics and is not partially
registered. Only reviewed dynamic actions with `exposure: :agent` enter
...
support in-place mutation of integrated source.
...
metadata, source hash, gate evidence, confirmation record, current settings,
and loader policy all match. If `dynamic_codegen.live_loader_enabled=false`,
boot reconciliation must not register dynamic authority surfaces. Disabling the
live loader while artifacts are live removes or blocks dynamic registrations
through the same audited authority-removal path as rollback.
...
- Security evals must prove untrusted core-load attempts, unscanned compile
  ...
  loader tampering, core-module replacement, action shadowing, emergency-disable
  bypass, restart reconciliation tamper, rollback failure, and exfiltration
  fail closed.
