...
- **Embedded scripting** (v0.9 Lua engine) is an opt-in advanced surface. It exists so Allbert — or a skill author — has a small, sandboxed DSL available. It is never a requirement for end-user workflows.
- **Self-improvement capabilities** (v0.9 Rust rebuild skill, skill-authoring skill) frame Allbert as the author, with the user as reviewer. The user approves diffs or install previews; the user does not produce the code.
