...
Proposed for v0.49b MCP Server Mode (`docs/plans/v0.49b-plan.md`).
...
The original v0.49 plan bundled MCP server with OpenAI-compatible HTTP API,
...
keeps the v0.49b ADR focused.

## Decision

v0.49b implements MCP server mode under the following rules:
...
- Tool/resource exposure is opt-in. An empty exposure is a valid 1.0
  configuration.
- The v0.50 security eval sweep covers MCP server mode (client/tool isolation,
