...
Beyond the shipped v0.12 self-improvement release, the roadmap first names a planned v0.12.1 operator-legibility point release and a planned v0.12.2 tracing/replay point release, then v0.13 personalization and v0.14 self-diagnosis. Those later releases are staged after self-improvement, UX polish, and trace/replay hardening so that Allbert's review surfaces, cost tracking, operator posture, and diagnostic history are mature before the assistant edits durable personalization state or reasons about its own failures.

- **v0.12.2 — Tracing and replay.** A point release after v0.12.1 persists session-local spans at `sessions/<id>/trace.jsonl`, exposes redacted replay through CLI/TUI/Telegram surfaces, and adds file-based OTLP-JSON export for operators who already own an observability stack. It is planned work, not shipped behavior; it makes v0.14 self-diagnosis consume stable trace artifacts instead of inventing trace persistence later.
- **v0.13 — Local personalization.** A `PersonalityAdapterJob` plugs into the v0.11 `LearningJob` seam and trains a small local adapter (LoRA or equivalent) from approved durable memory, approved facts, bounded recent episode summaries labelled as working-history-derived input, `SOUL.md` baseline persona/constraints, and accepted `PERSONALITY.md` learned adaptation input. Every new adapter routes through an `adapter-approval` inbox flow that mirrors v0.12's patch review. No training data leaves the machine unless the operator explicitly opts in. `PERSONALITY.md` remains the human-readable learned overlay; the adapter is a second, optional surface.
