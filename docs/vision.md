...
Beyond the shipped v0.12 self-improvement, v0.12.1 operator-legibility, and v0.12.2 tracing/replay releases, the roadmap next names v0.13 personalization and v0.14 self-diagnosis. Those later releases are staged after self-improvement, UX polish, and trace/replay hardening so that Allbert's review surfaces, cost tracking, operator posture, and diagnostic history are mature before the assistant edits durable personalization state or reasons about its own failures.
