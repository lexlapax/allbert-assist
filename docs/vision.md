...
Beyond the shipped v0.12 self-improvement release, the roadmap first names a planned v0.12.1 operator-legibility point release, then two further planned releases. Those later releases are staged after self-improvement and its UX polish so that Allbert's review surfaces, cost tracking, and operator posture are mature before the assistant edits durable personalization state or reasons about its own failures.
