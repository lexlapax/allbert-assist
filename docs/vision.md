...
- layering semantic retrieval as a derived index alongside BM25 rather than replacing markdown ground truth; v0.11 ships the seam with a fake deterministic provider first, and v0.15 ships real local-vector RAG with Ollama embeddings, `sqlite-vec`, SQLite FTS fallback, operator docs, commands, settings, memory, facts, episode recall, session-derived history, and collection-aware user RAG over trusted local paths plus explicit HTTP(S) URL sources before future growth-loop ingestion expands the corpus
...
Full foundation-model retraining, hosted web surfaces, ambient activity ingestion, and a broader distributed-service platform remain out of scope until their parked stubs in [docs/plans/future-plans.md](plans/future-plans.md) or the roadmap receive a focused design pass.
