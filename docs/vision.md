...
Beyond the shipped v0.12 self-improvement, v0.12.1 operator-legibility, v0.12.2 tracing/replay, and v0.13 personalization releases, the roadmap next names v0.14 self-diagnosis. That Draft release is staged after self-improvement, UX polish, trace/replay hardening, and local personalization so that Allbert's review surfaces, cost tracking, operator posture, diagnostic history, and self-change envelope are mature before the assistant reasons about its own failures.
