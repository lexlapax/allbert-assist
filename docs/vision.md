...
- **v0.13 — Local personalization.** A `PersonalityAdapterJob` plugs into the v0.11 `LearningJob` seam and trains a small local adapter (LoRA or equivalent) from approved durable memory, approved facts, and bounded recent episode summaries labelled as working-history-derived input. Every new adapter routes through an `adapter-approval` inbox flow that mirrors v0.12's patch review. No training data leaves the machine unless the operator explicitly opts in. `PERSONALITY.md` remains the human-readable profile; the adapter is a second, optional surface.
