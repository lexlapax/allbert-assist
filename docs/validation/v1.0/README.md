...
| DIT-4: Live advanced-surface regression | browser research + delegate, configured remote channels (in/out), MCP/OpenAI/ACP, Plan/Build approval | PASS (2026-07-14) | `dit4-advanced.md` — classes (a) browser, (b) telegram out+in, (c) MCP/OpenAI/ACP PASS live; class (d) failed pre-fix (R8/R9 → plan M7.1/M7.3), re-run PASS post-fix on the default router config with in-session typed-approval resolution and a completed run |
...
- DIT-2 true-non-developer no-docs first-run + one-click download from an empty cache
  (on the M7.x-fixed local build; the first two attempts surfaced R1-R3 and R10/R11,
  all fixed)
