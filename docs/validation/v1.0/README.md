...
| DIT-4: Live advanced-surface regression | browser research + delegate, configured remote channels (in/out), MCP/OpenAI/ACP, Plan/Build approval | **FAIL — packaged revalidation 2026-07-15** (source-checkout PASS 2026-07-14 superseded for release-artifact claims) | `dit4-advanced.md` §"Packaged v1.0.0 revalidation" — (a) FAIL (turn completed with only "handoff proposed", nothing ran), (b) FAIL/incomplete (ask misrouted to `external_network_request` `:missing_url`), (c) PARTIAL (MCP/OpenAI PASS; no packaged ACP handshake command), (d) FAIL/BLOCKED (packaged `allbert tui` crash pre-prompt). Remediation: v1.0.1 M4.1–M4.4; fix re-attestation 2026-07-15 (local 1.0.1 build): (b)/(c)/(d) PASS; (a) took three more passes — M4.2.1 (ladder routing), M4.2.2 (approval resume, superseded), then the operator-directed system redesign M4.2.3 (single up-front consent recording the durable navigation grant, per the workflow_run_start precedent; web typed-callback interception; live pending queue; objective origin attribution). Class (a) re-attests packaged §I (ONE approval expected) before the v1.0.1 tag |
