...
| DIT-2: No-docs first-run (historical v1.0 criterion; superseded for v1.2+) | v1.0: install → QuickStart → confirmed model download → first useful chat. v1.2 successor: install → open chat → first question with zero setup clicks when usable; otherwise working deterministic chat + exactly one confirmed repair path | PASS (2026-07-14) | Historical proof: `dit2-no-docs.md`. Shipped v1.2 proof is FV-01/FV-02 in `docs/plans/archives/v1.2-request-flow.md`; wizard completion is no longer an availability gate. |
