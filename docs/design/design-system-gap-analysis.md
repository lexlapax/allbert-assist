...
| Model/readiness status variants | `models`, `onboarding`, `workspace`; M3 first-model-state. | `status_badge` tones exist, but model readiness states are not encoded as reusable variants. | Add model-state badges/cards for the six as-built probe states: local_ready, runtime_missing, runtime_unhealthy, model_missing, below_hardware_floor, byok_ready; use a separate repair/blocked pattern for broader product blockers. |
...
| Model setup repair pattern | M6 `models`; M3 first-model-state. | Runtime missing, model missing, hardware floor, BYOK fallback, and no-path repair outcomes need one repair language. | Add model-readiness repair pattern with status, next step, fallback, and egress warning slots. |
