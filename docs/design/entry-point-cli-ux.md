...
| `allbert licenses` | Inspect the immutable license evidence packaged in this target artifact. | `mix allbert.licenses --check` validates the source catalog/union. | Additive v1.2.2 informational leaf; dispatched before Req, daemon attachment, or application startup. |
