...
| v0.61b (0.61.1) | Planned pre-1.0 UX-refinement point release: implements the eight operator feedback items from v0.61 manual validation — navigation consolidation (one sidebar with contextual workspace sections, per-shell top bars retired for per-view headers, workspace tool pane docked as a resizable split pane, icon-rail + full-hide collapse) plus chat type-scale fix, labeled status link-chip, renamable threads (registered `rename_thread` action, existing `:conversation_write`), and a subtler dark mode within Direction C. No new authority/permission/Settings key; does not renumber v0.62-v0.64. Gate `release.v061b` incl. the `:v061` regression proof. | ADR 0080 (Proposed → Accepted at S2), `docs/design/consolidated-shell-spec.md` (M0), v0.61b docs. |
