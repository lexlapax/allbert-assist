...
- v0.57 (in progress; M0-M9 implemented; awaiting warm TUI operator validation):
  ...
  **six-tool** default (read-only/sensitive read/grep/glob + effectful
  ...
  `Resources.Grants`, assistant-text token streaming + progressive diff streaming
  over the v0.55 static split on a new async turn-execution boundary enabling real
  Esc-to-cancel + queued steering, coding slash set with
  ...
  `coding.turn.supervised=false`. M6 added TUI coding-mode Esc/provider-stream
  cancellation through a registered stream cancel callback, bounded registry task
  shutdown, `turn_cancelled`, partial traces, and one-slot queued steering. M7
  ...
  `/init` over registered read/write actions with TUI approval handoff.
  ...
  bounded receipts, hashes, and confirmation summaries.
