...
| Reviewed long-term Memory | ADR 0089, `docs/operator/active-memory.md`, active plan | Canonical claim streams are Markdown-first authority; projections are disposable and every prompt candidate is canonically reauthorized. |
| Conversation Search and canonical deletion | ADR 0092/0093, `docs/developer/surface-contract.md`, `docs/operator/conversation-search.md` | Search is one central lexical read projection across surfaces; Corpus owns eligibility/deletion and reauthorizes every result. |
| Recurring and managed Jobs | ADR 0092 §7, relevant operator subsystem guide, active plan | One Jobs engine owns schedules, dirty kicks, pause/resume, admission, and history; features do not add private schedulers. |
| Objectives and long-running work | ADR 0021, `docs/plans/archives/v0.24-plan.md`, `docs/research/objective-runtime-research.md` | Use shared objective runtime; no private durable goal loops. |
| Apps, plugins, packs, surfaces, catalog | ADR 0015, ADR 0017, ADR 0023, ADR 0024, ADR 0030, ADR 0098 | App/pack metadata and descriptors do not grant authority. From v1.4, kernel and named OTP packs use the validated contribution contract; legacy declared plugins remain a compatibility input, not executable authority. |
...
| Cross-surface consistency | ADR 0073, `docs/developer/surface-contract.md`, `docs/plans/archives/v0.58-plan.md` | One renderer, event/audit by `surface_id`, identity resolution, action-backed reads. |
...
| Model recommendations and answering-head qualification | ADR 0072, ADR 0097, `docs/operator/model-recommendations.md`, active plan | Recommendations and the offline deterministic bar inform operators; neither grants authority, changes a default, or enters the runtime request path. The owner is `AllbertAssist.DevGates.V131HeadQualification` plus its focused test and `mix allbert.test qualify-head` surface. |
...
| Public contract transition (1.0 → 1.4) | `docs/developer/public-contract-freeze.md`, ADR 0098, active v1.4 M15/M17 | `release.v1` is the v1.4 migration comparison. Accepted v1.4 promotes component-owned `release.v14` plus affected-component selection as current test authority; external Tier-1 1.x compatibility remains adapter-owned. |
| Release planning & backlog lifecycle | `docs/plans/roadmap.md`, `docs/plans/future-features.md`, `AGENTS.md` (Post-1.0 Release Model) | An item leaves future-features when the operator assigns it a roadmap slot. Agents record newly discovered candidates only in the active plan pending operator disposition. Every binary release plan carries an upstream-dependency-refresh milestone (roadmap Working Rules). |
...
| v0.63 (0.63.0) | Released (tagged `v0.63.0`, 2026-07-09) pre-1.0 product: guided onboarding wizard + repo-maintained user-category profiles/personas over the packaged entry points. 8 Locked Decisions (unify onboarding mechanisms; declarative `priv/` persona catalog; TUI+line-CLI terminal wizard; three-tier vault surfaced), M1–M7 + audit + operator-validation remediation, `:v063` eval rows; ADR 0069 + 0075 carry the build decisions + exact per-persona seed values. Onboarding upgraded existing scaffolding (`onboarding.ex` objective wizard + `first_run.ex` marker) plus greenfield personas on Settings Central. No new authority. | ADR 0069 (re-scoped), ADR 0075, v0.63 plan/request-flow, `release.v063`. |
| v0.64 (packaged 0.64.3; docs tag 0.64.5) | Corrective release arc after v0.64.1 curl validation: trusted install and non-developer first run, with curl fail-closed cosign verification, Homebrew formula/tap freshness, package-first docs, service-first onboarding, repairable first-run states, guided local model setup, backup restore, startup migration serialization for concurrent fresh-Home first commands, cross-app version consistency gating, live web model-pull progress, and source/docs release-gate hardening. v0.64.5 is source/docs only. No new authority or Settings key. | v0.64 plan/request-flow, ADR 0076/0078, `release.v064`. |
| v0.65 (packaged 0.65.0) | Released `v0.65.0` (2026-07-11) after implementation + remediation M8.1-M8.6 (incl. operator manual-validation chat memory-write/recall fixes). Full-scope local knowledge launch path: config-free notes-root connect (`set_notes_root`, `allbert admin notes set-root PATH`), action-backed `workspace:notes` search/read, interactive `workspace:memory` review (keep/reject-as-`:flagged`/delete), scoped `admin memory status` with explicit `--all-users`, starter prompts, smoke coverage, and `release.v065` over the existing notes/files + active-memory engine. Resource refs are provenance; enforcement remains `PermissionGate` + root/extension bounding, confirmations, notes/files app scope, and no auto-memory promotion. | v0.65 plan/request-flow, `docs/design/local-knowledge-path.md`, v0.42 notes/files docs, active-memory docs, `release.v065`. |
...
| v1.3 | Released long-term user memory + Search Central, plus the adaptive fan-out correction: bi-temporal claim streams, review, archive, tombstone-first Forget, a canonical Corpus boundary, one FTS5 search projection, canonical conversation deletion, rules-derived manager admission, exact-counted compatibility, selective one-turn Jido workers, and durable report composition. | ADR 0089/0092/0093 plus ADR 0083/0021 amendments, archived v1.3 plan/request-flow, `release.v13`. |
| v1.3.1 | Shipped source-only at accepted SHA `7a27a9cc`; adds offline answering-head qualification plus bounded Memory full-build metadata and paused-managed-due corrections. Attended validation, `release.v131`, and the one aggregate passed. No package/Latest/tap movement; v1.4 is the next binary carrier. | ADR 0097 plus ADR 0089/0092 clarifications, archived v1.3.1 plan/request-flow. |
| v1.3.2 | Shipped source-only at accepted SHA `28dc39e0d`; adds preflight with exact-state refusal, executable semantic sentinels, fail-closed scope selection, immutable advisory compatibility, and central nil-default `model_roles.fast|capable|thinking` resolution. Independent review, attended validation, `release.v1`, `release.v132`, and the one aggregate passed. No package/Latest/tap movement; v1.4 is the next binary carrier. | ADR 0090/0088 plus archived v1.3.2 plan/request-flow. |
| v1.4 | In implementation. Spine enablers and the accepted kernel foundation: param-contract inventory re-verification, direct PermissionGate facade retirement, action-envelope consolidation, registry/settings/gate inversion, dependency closure before pure `apps/allbert_kernel` relocation, three pack extractions, and M15/M17 promotion of the component contract/test baseline. ADR 0046 admission found no real migration consumer, so no runner ships. | ADR 0046, ADR 0065, ADR 0098, v1.4 plan/request-flow. |
| v1.5 / v1.6 | Planned. Knowledge Central (LLM wiki) in two stages: a derived page graph over kept claims (v1.5), then document ingest, synthesis cache, schema, and composite query (v1.6). | ADR 0094/0095, v1.5 and v1.6 plan/request-flow. |
| v1.7 | Planned. Connectivity enablers: one OAuth substrate for email XOAUTH2 and hosted-LLM subscriptions, MCP 2025-11-25 parity, network hardening. | ADR 0096, v1.7 plan/request-flow. |
| v1.8 | Planned; readiness reset 2026-08-06. Adaptive usage profiling, one-click confirmed customization, descriptive observed outcomes, confirmed role remaps, and mobile stage 1. It consumes the preflight/scope infrastructure and model-role resolution shipped by v1.3.2; it owns neither. | ADR 0090, ADR 0084 amendment, v1.8 plan/request-flow. |
...
| **Before anything expensive** | Run `mix allbert.test preflight`; v1.3.2 M1 made it the under-two-minute mandatory phase and exact-state prerequisite for centrally classified expensive commands. |
