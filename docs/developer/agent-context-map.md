...
| v1.3.2 | Planned, source-only. Foundational enablers extracted from the profiling release on 2026-08-06 because everything depended on them and nothing depends on profiling: `mix allbert.test preflight` (with exact-state attestation, owner-CWD load plus tag/manifest reconciliation, immutable advisory second-Elixir probe, executable fixture sentinels, fail-closed `scope --base`) and central `model_roles.fast|capable|thinking` resolution for Settings.Models-owned task chains. | roadmap ladder item 9; active v1.3.2 plan/request-flow. |
...
| **Before anything expensive** | Until v1.3.2 implements `mix allbert.test preflight`, run the active plan's enumerated cheap equivalent. After v1.3.2 M1, the command is the under-two-minute mandatory phase. |
