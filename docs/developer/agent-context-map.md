...
| v0.65 | Planned Full-scope local knowledge launch path: config-free notes-root connect (`set_notes_root`, `allbert admin notes set-root PATH`), `workspace:notes`, interactive `workspace:memory` review (keep/reject-as-`:flagged`/delete), `admin memory status`, starter prompts, and `release.v065` over the existing notes/files + active-memory engine. Resource refs are provenance; enforcement remains `PermissionGate` + root/extension bounding, confirmations, and no auto-memory promotion. | v0.65 plan/request-flow, `docs/design/local-knowledge-path.md`, v0.42 notes/files docs, active-memory docs. |
