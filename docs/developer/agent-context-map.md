...
| v1.3.1 | Shipped source-only at accepted SHA `7a27a9cc`; adds offline answering-head qualification plus bounded Memory full-build metadata and paused-managed-due corrections. Attended validation, `release.v131`, and the one aggregate passed. First packaged by v1.4.0. | ADR 0097 plus ADR 0089/0092 clarifications, archived v1.3.1 plan/request-flow. |
| v1.3.2 | Shipped source-only at accepted SHA `28dc39e0d`; adds preflight with exact-state refusal, executable semantic sentinels, fail-closed scope selection, immutable advisory compatibility, and central nil-default `model_roles.fast|capable|thinking` resolution. Independent review, attended validation, `release.v1`, `release.v132`, and the one aggregate passed. First packaged by v1.4.0. | ADR 0090/0088 plus archived v1.3.2 plan/request-flow. |
| v1.4 | Released as immutable binary v1.4.0 at source/tag `9c82179e7`, with the official Homebrew tap at `775cbf8`. Establishes the kernel-owned Pack contract, thirteen extracted first-party Pack applications, registry/settings/gate inversion, direct PermissionGate facade retirement, consolidated action envelopes, and the component-owned `release.v14` contract/test authority that succeeds the v1.0 migration guard. ADR 0046 admission found no real migration consumer, so no runner ships. | ADR 0046, ADR 0065, ADR 0098, archived v1.4 plan/request-flow and `docs/validation/v1.4/`. |
