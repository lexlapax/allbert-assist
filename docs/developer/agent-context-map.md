...
| v1.3.1 | Shipped source-only at accepted SHA `7a27a9cc`; adds offline answering-head qualification plus bounded Memory full-build metadata and paused-managed-due corrections. Attended validation, `release.v131`, and the one aggregate passed. No package/Latest/tap movement; v1.8 is the next binary carrier. | ADR 0097 plus ADR 0089/0092 clarifications, archived v1.3.1 plan/request-flow. |
| v1.3.2 | Planned, source-only. Foundational enablers extracted from the profiling release on 2026-08-06 because everything depended on them and nothing depends on profiling: `mix allbert.test preflight` (with attestation, advisory second-Elixir probe, fixture-drift registry, fail-closed `scope --base`) and `model_roles.fast|capable|thinking`. | roadmap ladder item 9; v1.3.2 triad (not yet written). |
| v1.4 | Planned. Spine enablers **and the accepted kernel foundation**: settings migration runner plus consumers, full param-contract enforcement, PermissionGate deletion, action-envelope consolidation, registry/settings/gate inversion, `apps/allbert_kernel` with relocation, three pack extractions, and the **re-baselined public-contract freeze** taken at closeout. | ADR 0046, ADR 0065, one new kernel/pack/tier ADR (written at M16), the kernel-redo analysis §13, v1.4 plan/request-flow. |
...
| v1.8 | Planned; readiness reset 2026-08-06. Was implementation-ready as v1.4, but unbundling removed preflight/`model_roles` and its anchors now sit six releases upstream, so it needs a fresh readiness pass. Adaptive usage profiling, one-click confirmed customization, descriptive observed outcomes, per-role model profiles, mobile stage 1, and the preflight/scope release infrastructure at M0.5. The shipped v1.3.1 bridge is replaced only after accepted-tag M0 re-verification and equivalence proof. | ADR 0090, ADR 0084 amendment, v1.8 plan/request-flow. |
...
| **Before anything expensive** | Until v1.3.2 implements `mix allbert.test preflight`, run the active plan's enumerated cheap equivalent. After M0.5, the command is the under-two-minute mandatory phase. |
