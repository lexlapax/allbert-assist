...
| v0.62 | Planned pre-1.0 product: packaged `allbert` binary, unified CLI, daemon, ADR 0070 TUI convergence, OS secret-vault. Main lane is packaging/entry-point scope; only M0.1 carries small v0.61b post-audit web-preflight cleanup. Final readiness constraints: platform matrix decision at S2, local-only attach transport, M4 authority through `:command_execute` for installer execution, backup-before-migrate, all eight M0 proofs binding, S1 then M0 next. | ADR 0076, ADR 0070, v0.62 docs. |
