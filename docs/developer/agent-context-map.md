...
- v0.57 (in progress; M0-M5 complete): Pi-mode Coding Surface. A gated terminal coding surface with a
  **six-tool** default (read-only/sensitive read/grep/glob + effectful
  ...
  `coding.turn.supervised=false`; M6 still owns Esc/provider-stream cancellation
  and queued steering. Milestones M6-M9 remain.
