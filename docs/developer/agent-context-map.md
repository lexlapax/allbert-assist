...
| **Channel Pack 1 retro-validation (Telegram + email) + Channel Pack 2: Matrix + WhatsApp (Cloud API) + Signal (signal-cli daemon)**; implemented as `0.53.0` through M10 and ready for operator real-provider validation before tag; Viber documented on paper + deferred (standing fee), iMessage/SMS parked. Deepened to a one-large-release deep plan (M0-M10, constructs-first). **Opens with M5: the first real-provider live validation of the already-shipped Telegram + email channels (stub-only since v0.16) to Discord/Slack parity — per-provider external smokes, provider doctors, operator guides, live manual checks — before M6-M8 build the new adapters; pays down the stub-only debt that surfaced nine real bugs in v0.52.** Consumes v0.52 ADR 0016 primitives + ADR 0057 threading (`:reply_chain` WhatsApp/Signal, `:native_threads`/`:reply_chain` Matrix; Matrix `typed_command`/`link`/`list`, WhatsApp `button`/`typed_command`/`link`/`list`, Signal `typed_command`/`link`/`list`; `:list` is mandatory fallback). **Finishes v0.52's unbuilt system-wide constructs:** Key Custody (ADR 0058 — in-BEAM `:sensitive` decrypt-once secret custody, 0 new deps, + supervised `signal-cli` daemon via muontrap/erlexec, 0600 key files + preferred 0600 UNIX socket or loopback TCP/HTTP with auth/ACL controls; no locked-memory claim); channel trust-class gating (ADR 0059 — `trust_class` `:e2ee_origin`/`:server_readable`/`:local`, unified view excludes cross-channel E2EE-origin by default + audited opt-in, resume-downgrade confirmed, web/CLI retrofit); public signed webhook (ADR 0056 v0.53 amendment — WhatsApp `X-Hub-Signature-256` raw-body verify through a v0.51-ingress raw-body/pre-parser branch); descriptor `reply_key_type`/`quote_ttl_ms` consumption (Signal timestamp, WhatsApp 30-day degrade); phone-PII redaction + Signal ACI identity. Matrix = unencrypted rooms only (no Elixir E2EE). **Validation status (2026-06-16):** Telegram + email **delivery + inbound** live smokes passed (email fixed 3 IMAP/SMTP bugs; Telegram 0 code bugs); the **channel *approval* check is BLOCKED on v0.54** (router dead-end found in live validation) and **resumes after v0.54 M9/M10 + validation**; Matrix/WhatsApp/Signal real-provider live smokes still pending before tag | ADR 0016, ADR 0017, ADR 0056 (v0.53 amendment), ADR 0057, ADR 0058 (key custody/daemon), ADR 0059 (trust-class), ADR 0050 (deps: muontrap/erlexec), `docs/plans/v0.53-plan.md`, `docs/plans/v0.53-request-flow.md` | v0.53 / 0.53.0 |
| Intent deepening for chat-primary routing. **Two-stage local intent router** (ADR 0060: embedding prefilter → constrained-LLM disambiguation over a shortlist → confidence gate, default selector; deterministic ladder kept as fast-path + offline fallback) + **local embedding/router model tiers** (ADR 0061: nomic-embed-text + US-origin `router_local`=llama3.1:8b default + **local** `router_escalation_local`=gemma4:26b escalation, on by default/no egress/audited, hosted opt-in). **Removes the app-handoff channel dead-end** so a channel `confirmation: :required` action reaches approve/deny (the v0.53 approval blocker). Plus original deepening (ADR 0019/0034: classification, bounded multi-turn context, generalized disambiguation, TTL'd clarification turn-state). **M9 — descriptor lifecycle (ADR 0062):** coverage across the 192-action surface; the live registry returns 49 agent modules, with two internal-capability channel doctors to normalize/exclude, leaving the intended 47-action effective routable inventory (was 12 descriptors). Descriptors become self-maintaining through data-only YAML descriptor/vocabulary files, generation + SignalBus reindex hooks + operator YAML curation (app-module < action-module < generated < override), plus learned-review YAML proposals from reviewed memory / clarification / trace analysis that stay inert until promoted; `.exs` descriptor files and `Code.eval_file/1` are implementation drift. Includes `optimize_intent_descriptors` + `mix allbert.intent`, golden-set + bench; generated descriptors for dynamic/write-code actions inert until promoted; routable ≠ executable. **M10 — outbound compose (ADR 0063):** new `send_email`/`send_channel_message`/`create_calendar_event` (calendar via MCP discovery) effectful actions behind the confirmation gate, shared generic resume path, new `:email_send`/`:channel_message_send`/`:calendar_write` permissions, a `Channels.Outbound.send/4` boundary across all connected adapters; channel sends enforce identity-allowlist + trust-class gating before dispatch; routing grants no authority. **M9 + M10 are in v0.54 and gate the tag.** | ADR 0060, ADR 0061, ADR 0062, ADR 0063, ADR 0019, ADR 0034, ADR 0016/0056/0059 (outbound gating), `docs/plans/v0.54-plan.md`, `docs/plans/v0.54-request-flow.md` | v0.54 |
| Web UX redo: chat-primary `/workspace`, ephemeral→popups, canvas demoted, "Conversations" relabel (Surface substrate kept) | ADR 0023, ADR 0024, ADR 0015, `docs/plans/v0.55-plan.md`, `docs/plans/v0.55-request-flow.md` | v0.55 |
| Channel parity matrix + proper TUI/terminal channel under the ADR 0016 contract and ADR 0057 `threading: :rich` substrate | ADR 0016, ADR 0057, `docs/plans/v0.56-plan.md`, `docs/plans/v0.56-request-flow.md` | v0.56 |
| Release candidate hardening, export/import, settings schema migration, 1.0 tiered contract freeze | ADR 0046, `docs/plans/v0.57-plan.md`, `docs/plans/v0.57-request-flow.md`, `docs/plans/v1.0-plan.md`, `docs/plans/v1.0-request-flow.md` | v0.57-v1.0 |
...
- v0.45 (implemented as `0.45.0`): Marketplace Lite — data shape +
  ...
  submissions stay parked. Drafts ADR 0046 for v0.57.
...
- v0.53 (implemented as `0.53.0`; ready for live validation): Channel Pack 1 retro-validation (Telegram + email, first
  ...
  parked. One large release (M0-M10, constructs-first; M5 retro-validation before
...
- v0.54 (planned through M9/M10, M0-M8 implemented): Intent Deepening. The local
  two-stage router removes the v0.53 channel approval dead-end; M9 adds descriptor
  lifecycle/coverage/golden-set work (ADR 0062); M10 adds outbound compose actions
  for email, calendar, and channel send (ADR 0063). Model output stays advisory,
  and M9/M10 gate the tag.
...
- v0.56 (planned): Channel Parity + TUI. Explicit channel capability/parity
  matrix and a proper TUI/terminal channel under the ADR 0016 contract.
- v0.57 (planned): Hardening, Export/Import, Settings Migration, And Final RC.
  ...
  eval sweep over v0.40-v0.56, and gathers RC evidence.
