...
| Content-addressable artifact store (Artifacts Central; implemented through M2, M3-M7 planned): `artifact://sha256/<hex>` identity over an `<ALLBERT_HOME>/artifacts` object store (thin CAS on `:crypto` SHA-256 + sharded objects + atomic writes, no third-party store); type-agnostic durable artifacts uploaded by the operator, created by Allbert, or found via approved tools; provenance/MIME/byte/hash/retention metadata index with bytes never in traces; `artifact_read`/`artifact_write`/`artifact_delete` permissions + operation classes; `artifacts.*` settings fragment (`schema_version: 1`); `put`/`get`/`list`/`delete` actions plus the first supervised Jido ingestion sensor; an `artifact_thread_links` SQLite join table (role created_by/referenced_by) linking artifacts to the threads/messages that created them, from `context.request`, with by-thread + reverse query and idempotent deterministic link ids; backfills retained v0.48 audio, v0.49 vision-input, and v0.49 generated-image roots from existing retention-root settings while leaving ephemeral scratch and historical Browser cache outside M5; adds `:v050` artifact-store eval rows + `release.v050`; content-addressed identity and thread links never grant permission | ADR 0053, ADR 0054 (provenance + browser-surface split), ADR 0042 (artifact amendment), ADR 0031, ADR 0046, `docs/plans/v0.50-plan.md`, `docs/plans/v0.50-request-flow.md` | v0.50 |
| Artifacts Browser: operator browsing repository for Artifacts Central as a plugin/app (`plugins/allbert.artifacts/`, plugin id `allbert.artifacts`, modeled on StockSage + `allbert.browser`); a workspace `:canvas_panels` panel via `App`/`SurfaceProvider` + `workspace_panel_surfaces/1`, an `/apps/artifacts/<sha>` page LiveView (route in core router, module plugin-owned, sha validated before store reads), and a `mix allbert.artifacts` CLI; reads the store only through core `:artifact_read` actions (list incl. by-thread, get, `artifact_threads`, doctor) and deletes via the core confirmation-gated action; renders redacted metadata only; adds `:v050b` artifact-browser eval rows + `release.v050b` and deterministic browser-validation fixture seeding; grants no authority | ADR 0054, ADR 0015 (app/surface DSL), ADR 0017 (plugin contract), ADR 0024 (UI zones/page routes), `docs/plans/v0.50b-plan.md`, `docs/plans/v0.50b-request-flow.md` | v0.50b |
...
- v0.50 (M1-M2 implemented; M3-M7 planned): Artifacts Central. A uniform
  ...
  `put`/`get`/`list`/`delete` actions and the codebase's first supervised Jido
  ingestion sensor, links artifacts to the threads/messages that created them
...
- v0.50b (planned): Artifacts Browser. The operator browsing repository for
  ...
  redacted metadata only. Browse/search/filter by type, origin, thread, and
  date. Adds `:v050b` artifact-browser eval rows, `release.v050b`, and
  deterministic browser-validation fixture seeding. The plugin grants no
  authority and owns no store internals.
