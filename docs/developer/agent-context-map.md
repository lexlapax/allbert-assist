...
| v0.62b (0.62.1) | Planned point-release / docs-evidence follow-up for reusable release operations after v0.62.0: Homebrew tap fill, package-manager install proof, packaged TUI transcript, both Linux Docker rehearsals (`linux-arm64` native and `linux-x64` under `--platform linux/amd64`), and the durable evidence taxonomy. Does not renumber v0.63-v0.64 and adds no new authority, permission, Settings key, runtime action, trust semantic, or product capability. | v0.62b plan/request-flow, `docs/operator/release-rehearsal.md`, `docs/developer/test-strategy.md`. |
