...
- v0.57 (in progress; M0-M7 complete): Pi-mode Coding Surface. A gated terminal coding surface with a
  **six-tool** default (read-only/sensitive read/grep/glob + effectful
  ...
  Esc-to-cancel + queued steering, coding slash set with /model//clear//compact
  ungated) and a named **local-coding operator** trust tier
  ...
  audit, and `Resources.Grants` lifecycle. Milestones M8-M9 remain.
