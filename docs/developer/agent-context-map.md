...
| Public contract freeze (1.0) | `docs/developer/public-contract-freeze.md`, `docs/plans/archives/v1.0-plan.md`, ADR 0021 A20 | Tiered frozen contracts remain binding throughout 1.x; `mix allbert.test release.v1` is a stop condition. v1.4 adds a topology/ownership supplement, not a waiver or replacement. |
...
| v1.4 | Planned. Spine enablers and the accepted kernel foundation: param-contract inventory re-verification, direct PermissionGate facade retirement, action-envelope consolidation, registry/settings/gate inversion, dependency closure before pure `apps/allbert_kernel` relocation, three pack extractions, and a generated topology supplement to the still-binding v1 freeze. ADR 0046 admission found no real migration consumer, so no runner ships. | ADR 0046, ADR 0065, ADR 0098, v1.4 plan/request-flow. |
