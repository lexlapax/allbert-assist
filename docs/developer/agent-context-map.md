...
| v1.3.2 | Implementation in progress, source-only. M1–M3 landed preflight with exact-state refusal, executable semantic sentinels, fail-closed scope selection, immutable advisory compatibility, central nil-default `model_roles.fast|capable|thinking` resolution, and documentation/gate reconciliation. M3.1 independent review held attended validation; its batched systemic remediation and closure review are active before SV-0–SV-4. | roadmap ladder item 9; active v1.3.2 plan/request-flow. |
