...
| v1.3.2 | Implementation in progress, source-only. M1–M3 landed preflight with exact-state refusal, executable semantic sentinels, fail-closed scope selection, immutable advisory compatibility, central nil-default `model_roles.fast|capable|thinking` resolution, and documentation/gate reconciliation. M3.1's systemic remediation passed independent closure review; attended SV-0–SV-4 passed with the real local provider. Final gates/tag/closeout remain. | roadmap ladder item 9; active v1.3.2 plan/request-flow. |
| v1.4 | Planned. Spine enablers and the accepted kernel foundation: one proven explicit/reversible per-fragment migration, param-contract inventory re-verification, direct PermissionGate facade retirement, action-envelope consolidation, registry/settings/gate inversion, dependency closure before pure `apps/allbert_kernel` relocation, three pack extractions, and a generated topology supplement to the still-binding v1 freeze. | ADR 0046, ADR 0065, ADR 0098, v1.4 plan/request-flow. |
