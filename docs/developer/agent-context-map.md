...
- v0.57 (in progress; M0-M4 complete): Pi-mode Coding Surface. A gated terminal coding surface with a
  **six-tool** default (read-only/sensitive read/grep/glob + effectful
  ...
  rendering, and an Owl-backed `Channels.TUI.LiveRegion`; M5/M6 still own async
  turns and cancellation. Milestones M5-M9 remain.
