...
| Channel parity matrix + proper TUI/terminal channel under the ADR 0016 contract and ADR 0057 `threading: :rich` substrate, shipped as `0.55.0`: list-shaped channel identity map, a basic supervised `mix allbert.tui` launcher, prompt-stable scrollback rendering, and warm TUI validation accepted on 2026-06-22. Harvests Pi's split tool-result pattern as `model_payload` vs. `surface_payload` (ADR 0067 extending ADR 0029/0030) as the foundation for the v0.57 Pi-mode coding surface; v0.55 lands only the **static** split (a complete `surface_payload` render), **not** a live-region/streaming substrate — true streamed diff/token semantics are v0.57-owned (M4-M6, net-new). M1 also closes the v0.54-deferred Matrix generic outbound implementation behind `Channels.Outbound`; live Matrix provider smoke is blocked by inactive credentials, not by code. | ADR 0016, ADR 0067, ADR 0057, ADR 0029, ADR 0030, `docs/plans/v0.55-plan.md`, `docs/plans/v0.55-request-flow.md` | v0.55 / 0.55.0 |
...
| Pi-mode coding surface, in progress with M0-M9.2 implemented; warm operator validation remains before release closeout: a gated terminal coding surface with a **six-tool** default (read-only/sensitive read/grep/glob + effectful write/edit/bash through `Actions.Runner.run/3`), sub-1000-token prompt+tool-defs budget, chunked-read context discipline, on the one authority spine. **Coder-ergonomics parity** (vs Claude Code / Pi / Codex / Gemini CLI): coder-facing approval modes (default/accept-edits/plan/tier) as a confirmation-cost seam in `Security.Decision` (preserves decision/trace/audit) + per-repo "always allow" reusing the `Resources.Grants` remembered-grant lifecycle, live stream/cancel path (assistant-text token chunks + progressive tool-argument diff rendering over the v0.55 static split, async turn boundary, provider cancel, queued steering), coding slash set (/help /pi /mode /model /clear /init /diff /compact; /mode//model//clear//compact ungated, no `:coding_session_write`). Plus a named **local-coding operator** trust tier (ADR 0056 lineage, running at ADR 0009 **Level 1** — not "level 0") — never YOLO-default, never for channel-originated or generated-code sessions; `bash` is raw shell only at the tier (argv-only `run_shell_command` otherwise), cwd-jailed + timed out; modes/grants grant no authority; deterministic acceptance gates for effectful work (ordinary turn ends when the model stops calling tools); coding I/O is session-scoped (not long-term memory); Security Central intact. M0 accepted ADR 0068, registered the three coding permission atoms, defined stream-event vocabulary, and verified the v0.55 TUI/split-payload plus req_llm/Owl substrate. M1 registered `read`/`grep`/`glob` as `:coding_file_read` Runner actions with shared cwd-jail/path policy, ignore-aware direct Elixir search, binary handling, output caps, redaction, and relative-path payloads. M2 registered resumable `write`/`edit` as `:coding_file_write` Runner actions with confirmation-required file effects, cwd-jail destination checks, overwrite/symlink/binary/exact-match refusal, redacted split diff payloads, and Settings Central write/edit bounds. M3 registered resumable `bash` as a `:coding_shell_execute` Runner action backed by `Execution.CommandSpec`/`LocalRunner`, cwd jail, command/env allowlists, timeout/output caps, confirmation resume, raw shell only when the setting and local-coding tier both resolve, and sub-agent spawn denial. M4 added the authority-free stream pipeline and renderer: `ReqLLM.StreamChunk` -> `Coding.StreamEvent`, pure `Coding.StreamRenderer`, TUI `stream_events` fallback rendering, and an Owl-backed `Channels.TUI.LiveRegion`. M5 added `Coding.TurnSupervisor` over `AllbertAssist.TaskSupervisor` plus `Coding.TurnRegistry`, explicit coding-turn detection, partial `:cancelled`/`:timed_out` responses on shutdown/timeout, trace/persistence through the ordinary runtime path, and synchronous fallback via `coding.turn.supervised=false`. M6 added TUI coding-mode task cancellation/steering: Esc invokes a registered stream cancel callback when present, shuts down the registry task with `coding.cancel.grace_ms` plus hard-kill fallback, emits `turn_cancelled`, preserves partial traces, and applies one queued correction as the next coding turn. M7 wired Settings-backed local-coding tier resolution, approval modes, and hash-only remembered command grants through the `Security.Decision` confirmation-cost seam while preserving `:needs_confirmation` decisions, trace, audit, and `Resources.Grants` lifecycle. M8 added `Coding.Prompt`, `Coding.Session`, Settings-backed prompt/model keys, `/pi` realpath cwd-jail pinning, `/mode`, `/model`, `/clear`, `/compact`, `/diff`, `@file`, and `/init` over registered internal read/write actions with TUI approval handoff. M9 added `:v057` eval inventory rows, `mix allbert.test release.v057`, and split-payload hardening so TUI diff previews stay out of model-facing payloads. M9.1 moved all six coding actions out of `agent_modules/0`/public protocol exposure, added `Coding.SessionGuard` active-session + local-coding-tier denial before filesystem/shell work, and added `pi-mode-tools-denied-out-of-session-001`. M9.2 connects coding turns to `ReqLLM.stream_text`, incremental TUI stream events, and real `ReqLLM.StreamResponse.cancel` registration. | ADR 0068, ADR 0067, ADR 0056, ADR 0009, ADR 0016, `docs/plans/v0.57-plan.md`, `docs/plans/v0.57-request-flow.md`, `docs/operator/pi-mode-coding.md`, `docs/archives/pi-integration-rethink.md` | v0.57 |
| Surface Consistency, Settings Enforcement & Web Design System (rescoped 2026-06-24 from "Web UX Redo + Surface Policy" into the pre-1.0 consolidation release). **Four pillars on the existing spine:** (1) **Cross-Surface Contract (ADR 0073)** — every surface (web/TUI/channels/Mix/public-protocol/Pi-mode) is a thin uniform view: one `Surface.Renderer` (collapsing five bespoke renderers), uniform inbound/rejection/error event+audit by `surface_id`, `Identity.resolve` identity (no hardcoded `"local"`), operator reads through the ADR 0070 action layer (web too), shared invocation helpers; conformance matrix is the standing check. (2) **Settings enforcement** — all operator-tunable config through Settings Central; fix 6 known bypasses (trace env var, browser host-resolver, hardcoded intent `decisive_confidence`, active-memory/trace/ingestion limits) + CI guard; distinct from v0.59 migration (ADR 0046). (3) **Web Design System & UX language (ADR 0074)** — global tokens (color/type/spacing/motion), prop-driven component-variant registry, shared accessible pattern library, one app shell, catalog-as-boundary-for-ALL-pages (Jobs/Objectives folded in); chat-primary re-layout ("Conversations" relabel, modal ephemerals, canvas drawer) + Intents/Settings-Models/Surface-Policy panels executed on it; no model-generated UI, no internal rename, no new routes. (4) **Redundancy consolidation** — ~8 duplicated helper families (100+ sites) into shared modules. Operator surface policy (presentation governance only) preserved. M0–M15; fully-fleshed S0–S6 operator validation (browser + cross-surface parity + settings-no-bypass). Amends ADR 0016/0030/0070; ADR 0004/0031 settings note; ADR 0024 chat-primary revision. Hands a uniform/clean base to v0.59 (ADR 0065 param-contract, ADR 0046 migration, onboarding, RC). | ADR 0073, ADR 0074, ADR 0016, ADR 0030, ADR 0070, ADR 0004, ADR 0031, ADR 0023, ADR 0024, ADR 0015, ADR 0062, `docs/plans/v0.58-plan.md`, `docs/plans/v0.58-request-flow.md` | v0.58 |
...
- v0.55 (shipped as `0.55.0`): Channel Parity + TUI/Terminal Channel. Explicit
  ...
  `model_payload`/`surface_payload` split (ADR 0067 extending ADR 0029/0030),
  scrollback rendering, and the transient Owl status/live block that v0.57
  extends with its own coding diff renderer. Post-M4 audit corrections
...
- v0.57 (in progress; M0-M9.2 implemented; warm operator validation remains before
  release closeout):
  Pi-mode Coding Surface. A gated terminal coding surface with a
  **six-tool** default (read-only/sensitive read/grep/glob + effectful
  write/edit/bash through `Actions.Runner.run/3`), sub-1000-token prompt+tool-defs,
  chunked-read context, plus **coder-ergonomics parity** with Claude Code / Pi /
  Codex / Gemini CLI (approval modes default/accept-edits/plan/tier as a
  `Security.Decision` confirmation-cost seam + per-repo "always allow" reusing
  `Resources.Grants`, live stream/cancel path for assistant-token chunks +
  progressive diff rendering over the v0.55 static split on a new async
  turn-execution boundary, provider cancel + queued steering, coding slash set with
  /help//pi//mode//model//clear//init//diff//compact
  (/mode//model//clear//compact ungated) and a named **local-coding operator** trust tier
  (ADR 0056 lineage at ADR 0009 **Level 1** — not "level 0"; ADR 0068/0067) —
  never YOLO-default, `bash` raw shell only at the tier, modes/grants grant no
  authority, coding I/O session-scoped, deterministic acceptance gates intact.
  M0 accepted ADR 0068 and verified/registers the contract substrate; M1
  registered `read`/`grep`/`glob` as `:coding_file_read` Runner actions with
  shared cwd-jail/path policy, ignore-aware direct Elixir search, binary handling,
  output caps, redaction, and relative-path payloads. M2 registered resumable
  `write`/`edit` as `:coding_file_write` Runner actions with confirmation-required
  file effects, cwd-jail destination checks, overwrite/symlink/binary/exact-match
  refusal, redacted split diff payloads, and Settings Central write/edit bounds.
  M3 registered resumable `bash` as a `:coding_shell_execute` Runner action backed
  by `Execution.CommandSpec`/`LocalRunner`, cwd jail, command/env allowlists,
  timeout/output caps, confirmation resume, raw shell only when the setting and
  local-coding tier both resolve, and sub-agent spawn denial. M4 added the
  authority-free stream pipeline and renderer: `ReqLLM.StreamChunk` ->
  `Coding.StreamEvent`, pure `Coding.StreamRenderer`, TUI `stream_events` fallback
  rendering, and an Owl-backed `Channels.TUI.LiveRegion`. M5 added
  `Coding.TurnSupervisor` over `AllbertAssist.TaskSupervisor` plus
  `Coding.TurnRegistry`, explicit coding-turn detection, partial
  `:cancelled`/`:timed_out` responses on shutdown/timeout, trace/persistence
  through the ordinary runtime path, and synchronous fallback via
  `coding.turn.supervised=false`. M6 added TUI coding-mode task cancellation
  through a registered stream cancel callback when present, bounded registry task
  shutdown, `turn_cancelled`, partial traces, and one-slot queued steering. M9.2
  connects production coding turns to `ReqLLM.stream_text`, incremental TUI stream
  events, and real `ReqLLM.StreamResponse.cancel` registration. M7
  wired Settings-backed local-coding tier resolution, approval modes, and
  hash-only remembered command grants through the `Security.Decision`
  confirmation-cost seam while preserving `:needs_confirmation` decisions, trace,
  audit, and `Resources.Grants` lifecycle. M8 added `Coding.Prompt`,
  `Coding.Session`, Settings-backed prompt/model keys, `/pi` realpath cwd-jail
  pinning, `/mode`, `/model`, `/clear`, `/compact`, `/diff`, `@file`, and
  `/init` over registered internal read/write actions with TUI approval handoff.
  M9 added `:v057` EvalInventory rows, `test/security/v057_coding_eval_test.exs`,
  `mix allbert.test release.v057`, and split-payload hardening so TUI diff
  previews stay in `surface_payload` while model-facing payloads carry only
  bounded receipts, hashes, and confirmation summaries. M9.1 moved all six coding
  actions out of `agent_modules/0`/public protocol exposure, added
  `Coding.SessionGuard` active-session + local-coding-tier denial before
  filesystem/shell work, and added `pi-mode-tools-denied-out-of-session-001`.
- v0.58 (planned): Surface Consistency, Settings Enforcement & Web Design System
  (rescoped 2026-06-24 into the pre-1.0 consolidation release). Four pillars on the
  existing spine: (1) Cross-Surface Contract (ADR 0073) — every surface a thin
  uniform view (one renderer, uniform event+audit by `surface_id`, `Identity.resolve`,
  operator reads via the ADR 0070 action layer on the web too, shared invocation
  helpers); (2) settings enforcement — all operator-tunable config through Settings
  Central, 6 known bypasses fixed + CI guard (distinct from v0.59 ADR 0046 migration);
  (3) Web Design System & UX language (ADR 0074) — global tokens, prop-driven variant
  registry, shared pattern library, one shell, catalog-for-all-pages (Jobs/Objectives
  folded in), with the chat-primary re-layout + Intents/Models/Surface-Policy panels
  on it; (4) redundancy consolidation. Operator surface policy preserved as
  presentation governance. Amends ADR 0016/0030/0070; ADR 0004/0031 settings note;
  ADR 0024 revision. Hands a uniform/clean base to v0.59.
