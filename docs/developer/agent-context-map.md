...
| **Channel Pack 1 retro-validation (Telegram + email) + Channel Pack 2: Matrix + WhatsApp (Cloud API) + Signal (signal-cli daemon)**; implemented as `0.53.0` through M10. Telegram + email delivery/inbound live smokes passed; approval checks resume after the `v0.54.0` tag because the router prerequisite now exists. Matrix/WhatsApp/Signal real-provider live smokes remain pending before the v0.53 tag. `:list` remains the mandatory fallback primitive. Matrix scope stays unencrypted rooms only. | ADR 0016, ADR 0017, ADR 0056 (v0.53 amendment), ADR 0057, ADR 0058, ADR 0059, ADR 0050, `docs/plans/v0.53-plan.md`, `docs/plans/v0.53-request-flow.md` | v0.53 / 0.53.0 |
...
- v0.53 (implemented as `0.53.0`; approval validation resumes after v0.54 tag): Channel Pack 1 retro-validation (Telegram + email, first
