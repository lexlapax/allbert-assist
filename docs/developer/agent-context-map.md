...
| Channel parity matrix + `mix allbert.channels status` + proper TUI/terminal channel under the ADR 0016 contract and ADR 0057 `threading: :rich` substrate; TUI identity uses the existing list-shaped channel identity map. Harvests Pi's split tool-result pattern as `model_payload` vs. `surface_payload` (ADR 0067 extending ADR 0029/0030) as the foundation for the v0.57 Pi-mode coding surface; v0.55 lands the split/live-region substrate, not true streamed diff/token semantics. M1 also closes the v0.54-deferred Matrix generic outbound implementation behind `Channels.Outbound`. | ADR 0016, ADR 0067, ADR 0057, ADR 0029, ADR 0030, `docs/plans/v0.55-plan.md`, `docs/plans/v0.55-request-flow.md` | v0.55 |
...
- v0.55 (planned): Channel Parity + TUI/Terminal Channel. Explicit channel
  capability/parity matrix, `mix allbert.channels status`, Matrix generic
  outbound gap closure, and a proper TUI/terminal channel under the ADR 0016
  contract using the shared list-shaped identity map; harvests Pi's
