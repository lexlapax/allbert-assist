...
- v0.50 (planned): Artifacts Central. A uniform content-addressable store for
  ...
  v0.49 generated-image roots while leaving ephemeral scratch and historical
  Browser cache outside M5. A thin CAS over BEAM primitives, not a third-party
  store; content-addressed identity and thread links never grant permission.
- v0.50b (planned): Artifacts Browser. The operator browsing repository for
  ...
  date. The plugin grants no authority and owns no store internals.
