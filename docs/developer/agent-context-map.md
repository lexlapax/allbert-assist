...
| Channel Pack 2: WhatsApp, Signal, Matrix (iMessage parked); consumes v0.52 ADR 0016 approval primitives and ADR 0057 threading (`:reply_chain` for WhatsApp/Signal, `:native_threads`/`:reply_chain` for Matrix) rather than adding a mobile-specific conversation model | ADR 0016, ADR 0017, ADR 0057, `docs/plans/v0.53-plan.md`, `docs/plans/v0.53-request-flow.md` | v0.53 |
...
- v0.53 (planned): Channel Pack 2 - WhatsApp, Signal, And Matrix. iMessage
  parked (macOS-only platform constraint). Consumes the v0.52 approval-primitive
  and threading contracts rather than inventing a mobile-channel conversation
  model.
