...
| v1.3.2 | Implementation in progress, source-only. M1 has landed `mix allbert.test preflight` with exact-state attestation/refusal, owner-CWD load plus separate tag/manifest reconciliation, executable fixture sentinels, fail-closed `scope --base`, and the immutable advisory second-Elixir probe. M2 has landed central nil-default `model_roles.fast|capable|thinking` resolution for Settings.Models-owned task chains with concrete-chain parity. M3 documentation/gate rejoin, independent review, validation, and closeout remain. | roadmap ladder item 9; active v1.3.2 plan/request-flow. |
