...
| Pi-mode coding surface: a gated terminal coding surface with a **six-tool** default (four effectful boundary actions read/write/edit/bash + read-only ripgrep-backed grep/glob through `Actions.Runner.run/3`), sub-1000-token prompt+tool-defs budget, chunked-read context discipline, on the one authority spine. **Coder-ergonomics parity** (vs Claude Code / Pi / Codex / Gemini CLI): coder-facing approval modes (default/accept-edits/plan/tier) + per-repo "always allow", **net-new** live rendering (assistant-text token streaming + progressive tool-argument diff streaming over the v0.55 static split), Esc-to-cancel + queued steering, coding slash set (/help /model /clear /init /diff /compact). Plus a named **local-coding operator** trust tier (ADR 0056 lineage, running at ADR 0009 **Level 1** — not "level 0") — never YOLO-default, never for channel-originated or generated-code sessions; `bash` is raw shell only at the tier (argv-only `run_shell_command` otherwise), cwd-jailed + timed out; modes/allowlist grant no authority; deterministic acceptance gates for effectful work (ordinary turn ends when the model stops calling tools); Security Central intact. Builds on the v0.55 TUI channel, v0.55.1 console, and the static split payload. Milestones M0–M6. Readiness third pass 2026-06-23. | ADR 0068, ADR 0067, ADR 0056, ADR 0009, ADR 0016, `docs/plans/v0.57-plan.md`, `docs/plans/v0.57-request-flow.md`, `docs/archives/pi-integration-rethink.md` | v0.57 |
...
- v0.57 (planned): Pi-mode Coding Surface. A gated terminal coding surface with a
  **six-tool** default (read/write/edit/bash + read-only grep/glob through
  `Actions.Runner.run/3`), sub-1000-token prompt+tool-defs, chunked-read context,
  plus **coder-ergonomics parity** with Claude Code / Pi / Codex / Gemini CLI
  (approval modes default/accept-edits/plan/tier + per-repo "always allow",
  assistant-text token streaming + diff streaming over the v0.55 static split,
  Esc-to-cancel + queued steering, coding slash set) and a named **local-coding
  operator** trust tier (ADR 0056 lineage at ADR 0009 **Level 1** — not "level 0";
  ADR 0068/0067) — never YOLO-default, `bash` raw shell only at the tier, modes
  grant no authority, deterministic acceptance gates intact. Milestones M0–M6.
