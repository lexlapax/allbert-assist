...
| v0.62 | Planned pre-1.0 product: packaged `allbert` binary, unified CLI, daemon, ADR 0070 TUI convergence, OS secret-vault. Main lane is packaging/entry-point scope; only M0.1 carries small v0.61b post-audit web-preflight cleanup. Second-pass readiness constraints include the platform matrix decision, attach transport, M4 authority contract, and backup-before-migrate semantics. | ADR 0076, ADR 0070, v0.62 docs. |
