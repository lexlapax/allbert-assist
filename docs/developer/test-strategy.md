...
more than one primary tag); the checker itself fails correctly (exit 1) — the
earlier "exits 0" reads were the documented `mix | tail` exit-masking trap. M8
...
Do not replay `release.v13` or `release.v131`; prove their frozen definitions
unchanged. Run Credo at workstream rejoins and Dialyzer after the all-feature
rejoin, repeating Dialyzer only for type-relevant remediation. `mix precommit`
remains a commit compatibility shortcut and is never release evidence.
...
`release.v121` is the exact ordered `release.v12` prefix plus six v1.2.5 steps:
...
v1.2.5 timing override does not execute it. At the clean pushed tag candidate,
