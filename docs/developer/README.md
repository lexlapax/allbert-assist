# Developer Docs

Developer docs explain implementation contracts, subsystem boundaries, and test
gates. Start with the compact maps, then open subsystem guides as needed.

## Start Here

- [Agent context map](agent-context-map.md) - subsystem routing and release context.
- [Test strategy](test-strategy.md) - lane taxonomy, release gates, and isolation rules.
- [Runtime boundary map](runtime-boundary-map.md) - runtime/action/channel boundaries.
- [Jido agent pattern](jido-agent-pattern.md) - when and how to use Jido-backed agents.

## Contracts

- [CLI mapping](cli-mapping.md) - mix task -> `allbert` command disposition mapping.
- [Surface contract](surface-contract.md) - cross-surface renderer, event, identity, and action contract.
- [Web design system](web-design-system.md) - tokens, variants, shell, catalog, and shared patterns.
- [Channel parity](channel-parity.md) - channel payload and surface parity expectations.
- [Public protocol surfaces](public-protocol-surfaces.md) - public protocol exposure boundaries.

## Runtime And Extension Systems

- [Active Memory](active-memory.md)
- [Artifact store](artifact-store.md)
- [Artifacts browser](artifacts-browser.md)
- [Browser plugin](browser-plugin.md)
- [Delegate agents](delegate-agents.md)
- [Dynamic plugin drafts](dynamic-plugin-drafts.md)
- [How to create an Allbert app](how-to-create-an-allbert-app.md)
- [Marketplace Lite](marketplace-lite.md)
- [MCP client](mcp-client.md)
- [Plan build](plan-build.md)
- [Portability](portability.md)
- [Provider capabilities](provider-capabilities.md)
- [Sandbox gate runner](sandbox-gate-runner.md)
- [Self-improvement](self-improvement.md)
- [Self-improvement trace index](self-improvement-trace-index.md)
- [Template patterns](template-patterns.md)
- [Vision and image generation](vision-and-image-generation.md)

## Channel And Approval Internals

- [Channel approval primitives](channel-approval-primitives.md)
- [Cross-channel threading](cross-channel-threading.md)

## Historical Reviews

- [Testing strategy review 2026-06-11](testing-strategy-review-06-11-26.md)
