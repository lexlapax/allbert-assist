...
- `workspace:create` — implemented operator Canvas destination with gallery,
  ...
  calls `scaffold_template` against the default `./plugins/<slug>` target, or
  `<ALLBERT_HOME>/template-smoke/<slug>/` when the server runs with
  `ALLBERT_TEMPLATE_SMOKE=1`, and denies existing roots; `--target` and
  `--force` remain CLI-only Mix task controls. Live-integration mode calls
