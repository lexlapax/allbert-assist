...
| v0.51 | Public protocol facades | Expected to expose Allbert registered actions and app memory namespaces as MCP tools/resources and provide OpenAI-compatible HTTP + ACP adapters under shared auth/redaction/confirmation policy. Text-first subset only: OpenAI/ACP image, audio, resource, filesystem-root, and client-supplied MCP-server payloads do not grant media/filesystem/MCP authority. |
